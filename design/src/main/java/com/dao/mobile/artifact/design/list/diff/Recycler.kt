package com.dao.mobile.artifact.design.list.diff
...
import com.dao.mobile.artifact.common.numbers.isZeroOrPositive
...
class Recycler
{
    abstract class Adapter<T : Parcelable, V : RecyclerView.ViewHolder>
        ...
        private val context: Context, private var list: MutableList<T>) : ListAdapter<T, V>(comparator)
    {
        ...
        fun itemRemove(list: SparseArray<T>?): Boolean
        {
            if(list != null && list.size() > 0)
            {
                ...
                if(startPosition.isZeroOrPositive())
                {
